        ...
        for contract in self.contracts:
            ...
            if not self.is_cap_words(contract.name):
                info = "Contract '{}' ({}) is not in CapWords\n".format(
                    contract.name, contract.source_mapping_str
                )

                json = self.generate_json_result(info)
                self.add_contract_to_json(
                    contract, json, {"target": "contract", "convention": "CapWords"}
                )
                results.append(json)
            ...
            for struct in contract.structures_declared:
                if not self.is_cap_words(struct.name):
                    ...
                    json = self.generate_json_result(info)
                    self.add_struct_to_json(
                        struct, json, {"target": "structure", "convention": "CapWords"}
                    )
                    results.append(json)
            ...
            for event in contract.events_declared:
                if not self.is_cap_words(event.name):
                    ...
                    json = self.generate_json_result(info)
                    self.add_event_to_json(
                        event, json, {"target": "event", "convention": "CapWords"}
                    )
                    results.append(json)
            ...
            for func in contract.functions_declared:
                if not self.is_mixed_case(func.name):
                    ...
                    json = self.generate_json_result(info)
                    self.add_function_to_json(
                        func, json, {"target": "function", "convention": "mixedCase"}
                    )
                    results.append(json)
            ...
            for var in contract.state_variables_declared:
                ...
                if var.is_constant is True:
                    ...
                    if not self.is_upper_case_with_underscores(var.name):
                        info = (
                            "Constant '{}' ({}) is not in UPPER_CASE_WITH_UNDERSCORES\n"
                        )
                        info = info.format(var.canonical_name, var.source_mapping_str)
                    ...
                    if not correct_naming:
                        ...
                        json = self.generate_json_result(info)
                        self.add_variable_to_json(
                            var, json, {"target": "variable", "convention": "mixedCase"}
                        )
                        results.append(json)
            ...
            for enum in contract.enums_declared:
                if not self.is_cap_words(enum.name):
                    ...
                    json = self.generate_json_result(info)
                    self.add_enum_to_json(
                        enum, json, {"target": "enum", "convention": "CapWords"}
                    )
                    results.append(json)
            ...
            for modifier in contract.modifiers_declared:
                if not self.is_mixed_case(modifier.name):
                    ...
                    info = info.format(
                        modifier.canonical_name, modifier.source_mapping_str
                    )
