            ...
            for f in contract.functions_and_modifiers_declared:
                for node in f.nodes:
                    ...
                    if (
                        ...
                        and node.context[self.KEY]["send_eth"]
                    ):
                        if not any(
                            n != node for n in node.context[self.KEY]["send_eth"]
                        ):
                            continue
                        read_then_written = []
                        ...
                        if read_then_written:
                            ...
                            finding_key = (
                                ...
                                tuple(
                                    sorted(
                                        list(node.context[self.KEY]["calls"]),
                                        key=lambda x: x.node_id,
                                    )
                                ),
                                tuple(
                                    sorted(
                                        list(node.context[self.KEY]["send_eth"]),
                                        key=lambda x: x.node_id,
                                    )
                                ),
                            )
                            ...
                            if finding_key not in result:
                                result[finding_key] = []
                            result[finding_key] = list(
                                set(result[finding_key] + finding_vars)
                            )
        return result
    ...
    def _detect(self):
        ...
        for (func, calls, send_eth), varsWritten in result_sorted:
            ...
            for call_info in calls:
                info += "\t- {} ({})\n".format(
                    call_info.expression, call_info.source_mapping_str
                )
            if calls != send_eth:
            ...
            for (v, node) in sorted(
                varsWritten, key=lambda x: (x[0].name, x[1].node_id)
            ):
            ...
            for call_info in calls:
                self.add_node_to_json(
                    call_info, json, {"underlying_type": "external_calls"}
                )
            ...
            for (v, node) in varsWritten:
                self.add_node_to_json(
                    node,
                    json,
                    {"underlying_type": "variables_written", "variable_name": v.name},
                )
