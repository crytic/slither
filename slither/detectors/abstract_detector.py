...
class AbstractDetector(metaclass=abc.ABCMeta):
    ...
    def _create_base_element(
        type, name, source_mapping, type_specific_fields={}, additional_fields={}
    ):
    ...
    def add_variable_to_json(self, variable, d, additional_fields={}):
        type_specific_fields = {"parent": self._create_parent_element(variable)}
    ...
    def add_enum_to_json(self, enum, d, additional_fields={}):
        type_specific_fields = {"parent": self._create_parent_element(enum)}
        element = self._create_base_element(
            "enum",
            enum.name,
            enum.source_mapping,
    ...
    def add_node_to_json(self, node, d, additional_fields={}):
        type_specific_fields = {"parent": self._create_parent_element(node)}
        ...
        if isinstance(source_mapping, tuple):
            ...
            source_id = next(
                (
                    ...
                    ) in self.slither.source_units.items()
