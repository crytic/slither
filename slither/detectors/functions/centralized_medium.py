...
class CentralizedRiskMEDIUM(AbstractDetector):
    """
    Detector for modifiers that return a default value
    """

    ARGUMENT = "centralized-risk-medium"
    HELP = "Modifiers that can return the default value"
    IMPACT = DetectorClassification.MEDIUM
    CONFIDENCE = DetectorClassification.HIGH
    WIKI = " "

    WIKI_TITLE = "Centralized Risk with function transfer Token"
        ...
        for c in self.contracts:
            ...
            for function in c.functions:
                if function.name.lower() in ["transfer","transferfrom"]:
                    continue
                if CentralizedUtil.check_function_type_if_critical_risk(function):
