...
class ConstCandidateStateVars(AbstractDetector):
    ...
    def _detect(self):
        ...
        all_functions = list(
            set([item for sublist in all_functions for item in sublist])
        )

        all_variables_written = [f.state_variables_written for f in all_functions]
        all_variables_written = set(
            [item for sublist in all_variables_written for item in sublist]
        )
        ...
        for v in constable_variables:
            info = "{} should be constant ({})\n".format(
                v.canonical_name, v.source_mapping_str
            )
