...
def find_variable(
    var_name, caller_context, referenced_declaration=None, is_super=False
):
    ...
    if is_super:
        ...
        functions = {
            f.full_name: f
            for f in contract.available_elements_from_inheritances(
                d, getter_available
            ).values()
        }
    else:
    ...
    if is_super:
        ...
        modifiers = {
            m.full_name: m
            for m in contract.available_elements_from_inheritances(
                d, getter_available
            ).values()
        }
    else:
        ...
        if is_compact_ast:
            ...
            if value:
                if "subdenomination" in expression and expression["subdenomination"]:
                    value = str(
                        convert_subdenomination(value, expression["subdenomination"])
                    )
                ...
                if (
                    ...
                    value = str(
                        convert_subdenomination(
                            value, expression["attributes"]["subdenomination"]
                        )
                    )
            elif value is None:
