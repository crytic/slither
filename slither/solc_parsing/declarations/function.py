...
class FunctionSolc(CallerContextExpression):
    ...
    def _parse_variable_definition(self, stmt: VariableDeclarationStatement, node: NodeSolc, scope: Union[Scope, Function]) -> NodeSolc:
        if len(stmt.variables) == 1:
            ...
            local_var.set_offset(stmt.src, self._function.compilation_unit)

            local_var_parser = LocalVariableSolc(local_var, stmt)
            self._add_local_variable(local_var_parser)

            new_node = self._new_node(NodeType.VARIABLE, stmt.src, scope)
    ...
    def _parse_break(self, stmt: Break, node: NodeSolc, scope: Union[Scope, Function]) -> NodeSolc:
        new_node = self._new_node(NodeType.BREAK, stmt.src, scope)
        link_underlying_nodes(node, new_node)
        return new_node
    ...
    def _parse_params(self, params: ParameterList):
        self.parameters_src.set_offset(params.src, self._function.compilation_unit)
    ...
    def _parse_returns(self, returns: ParameterList):
        self.returns_src.set_offset(returns.src, self._function.compilation_unit)
