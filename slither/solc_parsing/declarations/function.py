...
class FunctionSolc:
    ...
    def _parse_for_compact_ast(self, statement: Dict) -> (Optional[Dict], Optional[Dict], Optional[Dict], Dict):
    ...
    def _parse_for_noncompact_ast(self, statement: Dict) -> (Optional[Dict], Optional[Dict], Optional[Dict], Dict):
    ...
    def _parse_for(self, statement: Dict, node: NodeSolc) -> NodeSolc:
        ...
        if self.is_compact_ast:
            ...
            pre, cond, post, body = self._parse_for_noncompact_ast(statement)
        ...
        if node_condition:
            ...
            link_underlying_nodes(node_startLoop, node_endLoop)  # this is an infinite loop but we can't break our cfg
    ...
    def _remove_incorrect_edges(self):
        ...
        for node in self._node_to_nodesolc.keys():
            if node.type in [NodeType.STARTLOOP]:
                ...
                if len([son for son in node.sons if son.type != NodeType.ENDLOOP and son != node]) == 0:
                    continue
