    ...
    def _get_indirect_shadowing_information(contract):
        ...
        if indirect_shadows:
            for collision_set in sorted(indirect_shadows, key=lambda x: x[0][1].name):
                ...
                collision_steps = [
                    colliding_function.contract_declarer.name
                    for _, colliding_function in collision_set
                ]
                collision_steps = ", ".join(collision_steps)
                result.append(
                    f"'{collision_set[0][1].full_name}' collides in inherited contracts {collision_steps} where {winner} is chosen."
                )
        return "\n".join(result)
