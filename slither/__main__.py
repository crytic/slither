...
def wrap_json_detectors_results(success, error_message, results=None):
    ...
    return {"success": success, "error": error_message, "results": results_json}
...
def get_detectors_and_printers():
    ...
    for entry_point in iter_entry_points(group="slither_analyzer.plugin", name=None):
        ...
        if not all(issubclass(d, AbstractDetector) for d in plugin_detectors):
            raise Exception(
                "Error when loading plugin %s, %r is not a detector" % (entry_point, d)
            )

        if not all(issubclass(p, AbstractPrinter) for p in plugin_printers):
            raise Exception(
                "Error when loading plugin %s, %r is not a printer" % (entry_point, p)
            )
...
def main_impl(all_detector_classes, all_printer_classes):
    ...
    try:
        ...
        if os.path.isfile(filename) or is_supported(filename):
            (results, number_contracts) = process(
            ...
            if args.splitted and args.solc_ast:
                (results, number_contracts) = process_files(
                    filenames, args, detector_classes, printer_classes
                )
            else:
                for filename in filenames:
                    (results_tmp, number_contracts_tmp) = process(
                        filename, args, detector_classes, printer_classes
                    )
            ...
            raise Exception(
                "Unrecognised file/dir path: '#{filename}'".format(filename=filename)
            )
        ...
        if printer_classes:
            ...
            logger.info(
                "%s analyzed (%d contracts), %d result(s) found",
                filename,
                number_contracts,
                len(results),
            )
