...
#)
from slither.tools.properties.properties.ercs.erc721.properties.transfer import (
    ERC721_Transferable,
    #ERC20_Pausable,
)
...
ERC721_PROPERTIES = {
    "Transferable": PropertyDescription(ERC721_Transferable, "Test the correct tokens transfer"),
...
def generate_erc721(
    ...
    # Check if the contract is an ERC721 contract and if the functions have the correct visibility
    errors = _check_compatibility(contract)
    if errors:
        logger.error(red(errors))
        return

    erc_properties = ERC721_PROPERTIES.get(type_property, None)
    if erc_properties is None:
        logger.error(f"{type_property} unknown. Types available {ERC721_PROPERTIES.keys()}")
        return
...
def _initialization_recommendation(type_property: str) -> str:
    ...
    content += "\t\t// or users should be able to mint tokens\n"
...
def _check_compatibility(contract):
    errors = ""
    if not contract.is_erc721():
        errors = f"{contract} is not ERC721 compliant. Consider checking the contract with slither-check-erc"
        return errors
...
def _get_properties(contract, properties: List[Property]) -> Tuple[str, List[Property]]:
    ...
    if contract.compilation_unit.crytic_compile.type == PlatformType.TRUFFLE:
        solidity_properties += "\n".join([property_to_solidity(p) for p in ERC721_CONFIG])
