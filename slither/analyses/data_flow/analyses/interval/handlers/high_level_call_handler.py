...
from slither.analyses.data_flow.analyses.interval.managers.variable_info_manager import VariableInfoManager
...
class HighLevelCallHandler:
    ...
    def handle_high_level_call(
        ...
        try:
            ...
            logger.debug(f"Handling HighLevelCall: {result_variable_name} = {operation.function_name}")
            logger.debug(f"Variable object: {result_variable}, name: {result_variable_name}")

            # Create a RangeVariable based on the return type
            self._create_range_variable_for_call_result(domain, result_variable_name, result_variable_type)
    ...
    def _create_range_variable_for_call_result(
        ...
        if isinstance(var_type, ArrayType):
            ...
            range_variables = self.variable_info_manager.create_bytes_offset_and_length_variables(var_name)
            ...
            logger.warning(f"Unsupported return type {var_type} for HighLevelCall, creating placeholder")
            range_variable = RangeVariable(
                interval_ranges=[],
                valid_values=ValueSet(set()),
                invalid_values=ValueSet(set()),
        ...
        logger.debug(f"Added to domain state. Total variables: {len(domain.state.get_range_variables())}")
