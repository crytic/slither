...
test_slither() {
	...
	if ! slither "$1" --solc-disable-warnings --detect "$2" --json "$DIR/tmp-test.json"; then
	...
	# run slither detector on input file and save output as json
	if ! slither "$1" --solc-disable-warnings --detect "$2" --legacy-ast --json "$DIR/tmp-test.json"; then
		echo "Slither crashed"
		exit 255
	fi
