    ...
    def format(slither, patches, elements):
        for element in elements:
            if (element['additional_fields']['target'] == "parameter"):
                FormatNamingConvention.create_patch(slither, patches, element['additional_fields']['target'], element['name'], element['type_specific_fields']['parent']['name'], element['type_specific_fields']['parent']['type_specific_fields']['parent']['name'], element['source_mapping']['filename_absolute'],element['source_mapping']['start'],(element['source_mapping']['start']+element['source_mapping']['length']))
                ...
                FormatNamingConvention.create_patch(slither, patches, element['additional_fields']['target'], element['name'], element['name'], element['type_specific_fields']['parent']['name'], element['source_mapping']['filename_absolute'],element['source_mapping']['start'],(element['source_mapping']['start']+element['source_mapping']['length']))
            else:
            ...
            if contract.name == contract_name:
                for function in contract.functions:
                    if function.name == name:
                        in_file_str = slither.source_code[in_file].encode('utf-8')
                        old_str_of_interest = in_file_str[modify_loc_start:modify_loc_end]
                        m = re.match(r'(.*)'+"function"+r'(.*)'+name, old_str_of_interest.decode('utf-8'))
                        old_str_of_interest = in_file_str[modify_loc_start:modify_loc_start+m.span()[1]]
                    ...
                    for internal_call in node.internal_calls_as_expressions:
                        if (str(internal_call.called) == name):
                            ...
                            patch = {
                                ...
                                "old_string" : old_str_of_interest.decode('utf-8'),
                                "new_string" : old_str_of_interest.decode('utf-8')[0].lower()+old_str_of_interest.decode('utf-8')[1:]
                            }
                            if not patch in patches[in_file]:
                                patches[in_file].append(patch)
