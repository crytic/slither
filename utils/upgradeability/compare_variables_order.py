...
def compare_variables_order_implementation(v1, contract_name1, v2, contract_name2):
    ...
    logger.info(
        green(
            "Run variables order checks between implementations... (see https://github.com/crytic/slither/wiki/Upgradeability-Checks#variables-order-checks)"
        )
    )
    ...
    if contract_v1 is None:
        logger.info(
            red("Contract {} not found in {}".format(contract_name1, v1.filename))
        )
        exit(-1)
    ...
    if contract_v2 is None:
        logger.info(
            red("Contract {} not found in {}".format(contract_name2, v2.filename))
        )
        exit(-1)

    order_v1 = [
        (variable.name, variable.type)
        for variable in contract_v1.state_variables
        if not variable.is_constant
    ]
    order_v2 = [
        (variable.name, variable.type)
        for variable in contract_v2.state_variables
        if not variable.is_constant
    ]
    ...
    for idx in range(0, len(order_v1)):
        ...
        if len(order_v2) < idx:
            logger.info(
                red(
                    "Missing variable in the new version: {} {}".format(
                        v1_name, v1_type
                    )
                )
            )
        ...
        if (v1_name != v2_name) or (v1_type != v2_type):
            ...
            logger.info(
                red(
                    "Different variables between v1 and v2: {} {} -> {} {}".format(
                        v1_name, v1_type, v2_name, v2_type
                    )
                )
            )
...
def compare_variables_order_proxy(implem, implem_name, proxy, proxy_name):
    ...
    logger.info(
        green(
            "Run variables order checks between the implementation and the proxy... (see https://github.com/crytic/slither/wiki/Upgradeability-Checks#variables-order-checks)"
        )
    )
    ...
    if contract_implem is None:
        logger.info(
            red("Contract {} not found in {}".format(implem_name, implem.filename))
        )
        exit(-1)
    ...
    if contract_proxy is None:
        logger.info(
            red("Contract {} not found in {}".format(proxy_name, proxy.filename))
        )
        exit(-1)
    ...
    for idx in range(0, len(order_proxy)):
        ...
        if len(order_implem) <= idx:
            logger.info(
                red("Extra variable in the proxy: {} {}".format(proxy_name, proxy_type))
            )
            continue
        ...
        if (proxy_name != implem_name) or (proxy_type != implem_type):
            ...
            logger.info(
                red(
                    "Different variables between proxy and implem: {} {} -> {} {}".format(
                        proxy_name, proxy_type, implem_name, implem_type
                    )
                )
            )
        else:
            logger.info(
                yellow("Variable in the proxy: {} {}".format(proxy_name, proxy_type))
            )
    ...
    if not found:
        logger.info(
            green(
                "No variables ordering error found between implementation and the proxy"
            )
        )
