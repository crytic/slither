...
def compare_variables_order_implementation(v1, contract_name1, v2, contract_name2):
    logger.info(
        green(
            "Run variable ordering checks between the "
    ...
    for (var_type, var_name) in new_vars:
        logger.info(
            green(
                "New state variable in {}: {} {}".format(
                    contract_name2, var_type, var_name
                )
            )
        )
    ...
    for (var_type, var_name) in missing_vars:
        ...
        logger.info(
            red(
                "Missing state variable in {}: {} {}".format(
                    contract_name2, var_type, var_name
                )
            )
        )
...
def compare_variables_order_proxy(implem, implem_name, proxy, proxy_name):
    logger.info(
        green(
            "Run variable ordering checks between the proxy and "
    ...
    for (var_type, var_name) in vars_proxy:
        logger.info(
            yellow(
                "state variable defined in proxy {}: {} {}".format(
                    proxy_name, var_type, var_name
                )
            )
        )
    ...
    for (var_type, var_name) in extra_vars:
        ...
        logger.info(
            red(
                "Extra state variable in proxy {}: {} {}".format(
                    proxy_name, var_type, var_name
                )
            )
        )
    ...
    if not issues_found:
        logger.info(
            green("No variable ordering issues found between proxy and implementation")
        )
...
def __get_and_check_contracts_existence(source, name):
    ...
    if contract is None:
        logger.info(red("Contract {} not found in {}".format(name, source.filename)))
        exit(-1)
    return contract
...
def __check_vars_order(old_name, new_name, vars_v1, vars_v2):
    ...
    for idx in range(len(vars_v1)):
        if idx >= len(vars_v2) or vars_v2[idx] != vars_v1[idx]:
            ...
            msg = msg.format(
                new_name, old_name, idx + 1, new_name, v1_var_type, v1_var_name
            )

            if idx < len(vars_v2):
                v2_var_type, v2_var_name = vars_v2[idx]
                msg += ", found {} {} instead.".format(v2_var_type, v2_var_name)
            else:
                msg += "."
