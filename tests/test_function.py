...
def test_functions(select_solc_version):
    # pylint: disable=too-many-statements
    with select_solc_version("0.6.12"):
...
def test_function_can_send_eth(select_solc_version):
    with select_solc_version("0.6.12"):
        slither = Slither("tests/test_function.sol")
...
def test_reentrant(select_solc_version):
    with select_solc_version("0.8.10"):
...
def test_public_variable(select_solc_version):
    with select_solc_version("0.6.12"):
        slither = Slither("tests/test_function.sol")
