...
contract PancakeOracle is IPriceFeedsExt, Ownable {
    using SafeMath for uint112;
    using SafeMath for uint256;

    address public pairRef;
    uint256 public baseTokenIndex;

    constructor(address _pairRef, uint256 _baseTokenIndex) {
        pairRef = _pairRef;
    ...
    function latestAnswer() external view override returns (uint256 _price) {
        ...
        ) = IPancakePair(pairRef).getReserves();
    ...
    function setPairRefAddress(address _ref) public onlyOwner {
        pairRef = _ref;
    }

    function setBaseTokenIndex(uint256 _baseTokenIndex) public onlyOwner {
        baseTokenIndex = _baseTokenIndex;
    }
}