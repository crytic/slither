...
def test_struct_constructor_reorder(solc_binary_path) -> None:
    ...
    slither = Slither(Path(ARG_REORDER_TEST_ROOT, "test_struct_constructor.sol").as_posix(), solc=solc_path)
...
def test_internal_call_reorder(solc_binary_path) -> None:
    solc_path = solc_binary_path("0.8.15")
    slither = Slither(Path(ARG_REORDER_TEST_ROOT, "test_internal_call_reorder.sol").as_posix(), solc=solc_path)
    ...
    assert (
        ...
        and internal_calls[0].arguments[1].value == True
    )
    assert (
